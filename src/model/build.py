...
def get_backbone(model_cfg):
    if model_cfg.backbone.type == "cnn":
        return CustomResNet(model_cfg.backbone.name, out_features=model_cfg.backbone.out_features)
    elif model_cfg.backbone.type == "vit":
        return CustomSwinTransformer(model_cfg.backbone.name, out_features=model_cfg.backbone.out_features)
    else:
